...
SELECT *
FROM ST_Read('lab2/map.geojson')
WHERE "user" = 'Сахарова Дарья'
  AND starts_with(ST_AsText(geom), 'POLYGON')
...
osm_data_bbox AS (
    SELECT ST_Xmin(geom) - 0.003 AS xmin,
           ST_Ymin(geom) - 0.003 AS ymin,
           ST_Xmax(geom) + 0.003 AS xmax,
           ST_Ymax(geom) + 0.003 AS ymax
